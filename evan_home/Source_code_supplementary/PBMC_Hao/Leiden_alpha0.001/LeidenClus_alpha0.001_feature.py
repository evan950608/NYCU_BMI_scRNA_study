...
def read_data():
    ...
    # adata = sc.read('/Users/evanli/Documents/Research_datasets/PBMC_Hao/Hao_PBMC_level1_rep_cells.h5ad')
    print(adata.shape)
...
def run_tuning(adata, cts, label):
    for celltype in [cts]:
        ...
        server_path = '/home/evanlee/PBMC_Hao/Level_1_alpha0.001'
...
def pipeline_feature_selection(data, celltype, label, opt_lmbd, alpha, output_path=''):
    print('====================')
    print('Starting feature selection for {}'.format(celltype))
    # Binary classification of a celltype
    celltype_label = [1 if x == celltype else 0 for x in label]
    # create index for a celltype
    celltype_indices = [idx for idx, label in enumerate(celltype_label) if label == 1]
...
def run_pipeline_feature_selection(celltype):
    # read optimal lambda
    os.chdir('/home/evanlee/PBMC_Hao/Leiden_alpha0.001')
    with open(f'./cluster_{celltype}/{celltype}_opt_lambda.txt', 'r') as f:
        opt_lmbd = float(f.read())
    print(celltype, 'Optimal lambda:', opt_lmbd, '\n')

    # set learning rate alpha to 0.001
    pipeline_feature_selection(adata, celltype, label, opt_lmbd, alpha=0.001, output_path='/home/evanlee/PBMC_Hao/Leiden_alpha0.001')



# %% 
adata, cts, label = read_data()

# run_tuning(adata, cts, label)

# Create a pool of worker processes
with mp.Pool(processes=len(cts)) as pool:
    # Map the run_pipeline_ad2 function to each cell type using the pool of workers
