...
# %%
done_types = ['B_memory', 'CD16_Mono', 'CD4_CTL', 'CD4_TCM', 'NK']
listdir = ['log_CD8_Naive.txt', 'log_CD14_Mono.txt', 'log_CD4_Naive.txt', 'log_CD4_Proliferating.txt', 'log_CD8_TCM.txt', 'log_CD8_Proliferating.txt', 'log_B_intermediate.txt', 'log_CD4_TEM.txt', 'log_ASDC.txt', 'log_B_naive.txt']
listdir = [s.replace('log_', '').replace('.txt', '') for s in listdir]
done_types += listdir
print('done types:', done_types)
cts = []
for celltype in types:
    if celltype not in done_types:
        cts.append(celltype)
print('cts', cts)
print('====================')
...
for celltype in cts:
    ...
    server_path = '/home/jovyan/work/GitHub/EvanPys/Progress/PBMC_Hao/Level2_loss_converge/loss_lambda_tuning_remaining'
