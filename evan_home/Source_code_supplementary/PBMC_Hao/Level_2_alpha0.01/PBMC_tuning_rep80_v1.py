...
# read representative cells (top 20% clustering coefficient)
rep_cells = pd.read_csv('/home/evanlee/PBMC_Hao/PBMCHao_raw_representative_cells.csv')

# subset representative cells from raw adata
adata_rep = adata_raw[rep_cells['Cell'], :]

label = adata_rep.obs['celltype.l2'].tolist()
...
def process_celltype(celltype):
    st = time.time()
    pipeline_ad2(adata_rep, celltype, label, output_path='/home/evanlee/PBMC_Hao/AD2_result')
    et = time.time()
