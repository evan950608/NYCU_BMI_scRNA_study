...
def pipeline_ad2(data, celltype, label, output_path='', tuning_result=''):
    ...
    if tuning_result:
        ...
        Fig = ad.lambda_tuning_viz(result_dict, 'Precision met', savepath='{}_Precision2.png'.format(celltype))
...
for celltype in types:
    st = time.time()
    pipeline_ad2(adata, celltype, label, output_path='/home/evanlee/PBMC_Hao/Level_1_alpha0.01/Level1_result')
    et = time.time()
...
def run_pipeline_feature_selection(celltype):
    # read optimal lambda
    os.chdir('/home/evanlee/PBMC_Hao/Level_1_type/Level1_result')
    with open(f'./{celltype}_opt_lambda.txt', 'r') as f:
        opt_lmbd = float(f.read())
    print(celltype, 'Optimal lambda:', opt_lmbd, '\n')

    pipeline_feature_selection(adata, celltype, label, opt_lmbd, output_path='/home/evanlee/PBMC_Hao/Level_1_type/Level1_result')
