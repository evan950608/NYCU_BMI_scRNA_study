...
### Read DEG table
os.chdir('/ws/GitHub/EvanPys/Progress/PBMC_Hao_GSE/L2_by_lossdiff/L2_DEG/L2_DEG_table')
# celltype = 'ASDC'
...
def SVM_classifier_DEG_cuml(data, DEGn, celltype):
    ...
    del X, y, X_train, X_test, y_train, y_test, y_pred, y_test_cpu, y_pred_cpu
...
print('==================')
print('all metrics')
print(all_metrics_DEG)
os.chdir('/ws/GitHub/EvanPys/Progress/PBMC_Hao_GSE/L2_by_lossdiff/L2_DEG')
all_metrics_DEG.to_csv('L2_loss_diff_DEGn_SVM_rbf_metrics.csv')

# Plot metrics for each celltype
cols = ['Precision', 'F1-score', 'ROC-AUC', 'PR-AUC', 'MCC']
ax = all_metrics_DEG[cols].plot.bar(rot=0, figsize=(15,6), title='One vs. Rest SVM using DEGn (kernel=rbf)')
ax.legend(loc='center left', bbox_to_anchor=(1.0, 0.5))
plt.tight_layout()
plt.xticks(rotation=90)
plt.savefig('SVM_DEGn_L2_rbf_cuda.png', bbox_inches='tight')
