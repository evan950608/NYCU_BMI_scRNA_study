...
def pipeline_ad2(X_norm, X_raw_count, celltype, label, output_path='', tuning_result=''):
    ...
    if tuning_result:
        ...
        # Lambda tuning
        result_DF, result_dict, loss_history_dict, loss_diff_history_dict = ad.lambda_tuning_cuda(X_norm, X_raw_count, celltype_label, lmbd_range, device='cuda', training_echo=True)        
...
for celltype in queue:
    ...
    st = time.time()
    # Set output path
    server_path = '/home/jovyan/work/GitHub/EvanPys/Progress/PBMC_Hao_ztrans_weight/Level1'
