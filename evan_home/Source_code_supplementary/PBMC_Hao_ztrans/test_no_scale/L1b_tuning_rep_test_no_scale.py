...
def pipeline_ad2(X_norm, X_raw_count, celltype, label, output_path='', tuning_result=''):
    ...
    if tuning_result:
        ...
        # Lambda tuning
        result_DF, result_dict, loss_history_dict, loss_diff_history_dict = ad.lambda_tuning_cuda(X_norm, X_norm, celltype_label, lmbd_range, device='cuda', loss_tol=1e-6)        
...
# %%
adata = sc.read_h5ad('/home/jovyan/work/Research_datasets/PBMC_Hao/GSE164378_Hao/Harmony_and_ztrans/Hao_L1_repcells_test_no_scale.h5ad')
...
for celltype in queue:
    ...
    st = time.time()
    # Set output path
    server_path = '/home/jovyan/work/GitHub/EvanPys/Progress/PBMC_Hao_ztrans/test_no_scale'

    # set learning rate alpha to 0.01
    # pipeline_ad2(ad_X_sparse, celltype, label, output_path=server_path)
    pipeline_ad2(ad_X_sparse, ad_X_sparse, celltype, label, output_path=server_path)
    et = time.time()
