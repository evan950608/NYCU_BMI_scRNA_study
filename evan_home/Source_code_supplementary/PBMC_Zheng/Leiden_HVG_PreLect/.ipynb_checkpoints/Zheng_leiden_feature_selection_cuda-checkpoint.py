...
def pipeline_feature_selection(adata, X_sparse, celltype, label, opt_lmbd, output_path=''):
    ...
    opt_res = ad.ADlasso2(lmbd=opt_lmbd, loss_tol=1e-6, echo=True, device='cuda', max_iter=10)  # cuda
...
def run_pipeline_feature_selection(cluster):
    st = time.time()
    ...
    local_path = ''
    pipeline_feature_selection(adata, ad_X_sparse, celltype, label_leiden, opt_lmbd, output_path=server_fractal_path)
