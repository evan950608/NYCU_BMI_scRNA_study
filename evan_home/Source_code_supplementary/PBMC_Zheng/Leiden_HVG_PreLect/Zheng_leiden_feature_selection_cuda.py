...
def run_pipeline_feature_selection(cluster, additional_label):
    st = time.time()
    # Read adata again for each celltype
    adata = sc.read_h5ad('/home/jovyan/work/Research_datasets/PBMC_68k_Zheng/Zheng_PBMC.h5ad')
    print('Original adata:', adata.shape)
    adata.obs['leiden_cluster'] = additional_label
    
    ### Normalization
    # normalize_total to median of total counts for cells
    sc.pp.normalize_total(adata, target_sum=None)
    # log1p
    sc.pp.log1p(adata)

    ### Remove the genes whose expression is zero in all cells of this celltype
    adata_cluster = adata[adata.obs['leiden_cluster'] == cluster]
    print('adata celltype shape:', adata_cluster.shape)
    
    # Remove explicit zeros from the sparse matrix
    # adata_celltype.X.eliminate_zeros()

    # Find the columns that are all zeros
    all_zeros = np.where(adata_cluster.X.getnnz(axis=0) == 0)[0]
    # all_zeros = np.where(np.all(adata_celltype.X == 0, axis=0))[0]

    # Remove the columns that are all zeros from the anndata object
    adata = adata[:, ~adata_cluster.var_names.isin(adata_cluster.var_names[all_zeros])]
    print('adata shape after removing all zero columns for celltype cells:', adata.shape)
    del adata_cluster, all_zeros
    ...
    label_leiden = additional_label


    ### Read optimal lambda dictionary from json
    path_opt_lambda = '/home/jovyan/work/GitHub/EvanPys/Progress/PBMC_Zheng/Leiden_HVG_PreLect/Zheng_leiden_opt_lmbd.json'
    with open(path_opt_lambda, 'r') as f:
        opt_lambda_dict = json.load(f)
    opt_lmbd = opt_lambda_dict[cluster]
    print('optimal lambda:', opt_lmbd)

    server_fractal_path = '/home/jovyan/work/GitHub/EvanPys/Progress/PBMC_Zheng/Leiden_HVG_PreLect/feature_selection'
    local_path = ''
    pipeline_feature_selection(adata, ad_X_sparse, cluster, label_leiden, opt_lmbd, output_path=server_fractal_path)

    et = time.time()
    print(f'Elapsed time for {cluster}: {(et-st)/60:.2f} minutes')
    del adata
...
adata_leiden = sc.read_h5ad('/home/jovyan/work/Research_datasets/PBMC_68k_Zheng/Zheng_PBMC_HVG2000_leiden_subset.h5ad')
label_leiden = adata_leiden.obs['leiden_cluster'].tolist()
...
print('unique leiden clusters adata:', unique_leiden)
print('====================')
...
for cluster in queue:
    print('====================')
    print('Starting job for {}'.format(cluster))
    run_pipeline_feature_selection(cluster, label_leiden)
