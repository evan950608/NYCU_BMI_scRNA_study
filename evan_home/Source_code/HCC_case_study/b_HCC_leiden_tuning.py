...
print('all leiden clusters:', clusters)
print('====================')
...
for clus in clusters:
    ...
    st = time.time()
    # Set output path
    # server_path = '/home/jovyan/work/GitHub/EvanPys/Progress/HCC_case_study/tuning_leiden'
    server_path = source_code_dir / 'HCC_case_study/tuning_leiden'
    print('Valid output path?', os.path.exists(server_path))

    # set learning rate alpha to 0.01
    pipeline_ad2(ad_X_sparse, clus, label, output_path=server_path)
    et = time.time()
    print('{} Time elapsed: {} minutes.'.format(clus, (et-st)/60))
    del adata
