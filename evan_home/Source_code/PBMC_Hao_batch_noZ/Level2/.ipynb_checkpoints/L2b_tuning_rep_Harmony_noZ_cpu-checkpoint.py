...
def pipeline_ad2(X_norm, celltype, label, X_raw_count=None, output_path='', tuning_result=''):
    ...
    if tuning_result:
        ...
        # Lambda tuning
        result_dict, loss_history_dict, loss_diff_history_dict = ad.lambda_tuning_para_cpu(X_norm, celltype_label, lmbd_range, device='cpu', loss_tol=1e-6, max_iter=5)
        ...
        try:
            ...
            Fig = ad.lambda_tuning_viz(result_dict, 'Other_prevalence', savepath='{}_Train_prevalence.png'.format(celltype))
        except:
...
print('Representative adata:', adata.shape, type(adata.X))
...
for celltype in queue:
    ...
    adata = sc.read_h5ad('/home/jovyan/work/Research_datasets/PBMC_Hao/GSE164378_Hao/Harmony_noZ/Hao_L2_repcells_loginv_Harmony_noZ.h5ad')
    print('Representative adata:', adata.shape)
    ...
    st = time.time()
    # Set output path
    server_path = '/home/jovyan/work/GitHub/EvanPys/Progress/PBMC_Hao_batch_noZ/Level2'
