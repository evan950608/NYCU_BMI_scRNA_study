...
for celltype in types:
    ...
    filename = f'SVM_{celltype}_rbf_StardardScale_l2.pkl'
    with open(filename, 'wb') as f:
        pickle.dump(clf, f)

print(all_metrics)
all_metrics.to_csv('SVM_metrics_rbf_StandardScale_l2.csv')

# Plot metrics for each celltype
cols = ['Accuracy', 'Precision', 'F1-score', 'ROC-AUC', 'PR-AUC']
ax = all_metrics[cols].plot.bar(rot=0, figsize=(15,6), title='One vs. Rest SVM using PreLect features (Kernel=rbf)')
ax.legend(loc='center left', bbox_to_anchor=(1.0, 0.5))
plt.savefig("SVM_metrics_rbf_StandardScale_l2.png", format="png")