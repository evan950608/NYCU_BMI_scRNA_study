...
### Read features
import os
os.chdir('/home/jovyan/work/GitHub/EvanPys/Progress/PBMC_Hao_batch_noZ/Level1/feature_selection_k3')
...
### Stratified K-fold using DEGn
print('===== Stratified K-fold')
def SVM_strat_KFold(data, DEGn, celltype, k=5):  # all_features_dict
...
# os.chdir('/home/jovyan/work/GitHub/EvanPys/Progress/PBMC_Hao_batch_noZ/Level1/DEG_L1/L1_DEG_table')
...
for celltype in types:
    ...
    # print(DEG_table.head())
    DEGn = DEG_table['names'][:n_features].tolist()
    # print(DEGn)
    clf, metrics, cv_results = SVM_strat_KFold(adata, DEGn, celltype, k=5)  # metrics is a list
    print(metrics)
    ...
    filename = f'SVM_{celltype}_rbf_StardardScale_l1_DEG.pkl'
    with open(filename, 'wb') as f:
        pickle.dump(clf, f)

print(all_metrics)
all_metrics.to_csv('SVM_metrics_rbf_StandardScale_l1_DEG.csv')

# Plot metrics for each celltype
cols = ['Accuracy', 'Precision', 'F1-score', 'ROC-AUC', 'PR-AUC']
ax = all_metrics[cols].plot.bar(rot=0, figsize=(8,6), title='One vs. Rest SVM using top DEGs (Kernel=rbf)')
ax.legend(loc='center left', bbox_to_anchor=(1.0, 0.5))
plt.savefig("DEG_SVM_metrics_rbf_StandardScale_l1.png", format="png")