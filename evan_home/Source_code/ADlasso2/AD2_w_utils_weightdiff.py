...
def lambda_tuning_para_ttsplit(X_norm, Y, lmbdrange, device='cpu', training_echo=False,
    ...
    def test_lambda(lambd):
        ...
        examined_lambda = ADlasso2(lmbd = lambd, device = device, echo = training_echo, alpha=alpha, max_iter=max_iter, loss_tol=loss_tol, gamma=gamma)  # set alpha, gamma, max_iter, tol?
        examined_lambda.fit(train_X, train_y, train_pvl)
        selected_set = examined_lambda.feature_set
        ...
        return (percentage, prevalence_, train_prevalence, feature_number, 
...
def lambda_tuning_cuda(X_norm, X_raw_count, Y, lmbdrange, device='cuda', training_echo=False,
                                max_iter=1000, loss_tol=1e-6, weight_tol=1e-6, alpha=0.01, gamma=0.1):
    print('Alpha:', alpha)
    print('Weight tolerance:', weight_tol)
    class_content = np.unique(Y)
    if len(class_content) != 2:
        raise ValueError("This procedure allows only 2 classes.")
    ...
    test_index = np.where(np.logical_not(train_test_index))[0]
    train_X, test_X = X_norm[train_index], X_norm[test_index]
    train_X_raw_count, test_X_raw_count = X_raw_count[train_index], X_raw_count[test_index]
    train_y, test_y = y[train_index], y[test_index]
    ...
    def test_lambda(lambd):
        ...
        # Training local prevalence: Get the prevalence of one cell type in training set
        train_celltype_indices = [idx for idx, label in enumerate(train_y) if label == 1]
        train_pvl = get_prevalence(train_X_raw_count, train_celltype_indices)

        # Create new ADlasso object
        print('Testing lambda:', round(lambd, 6), 'starting at', datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Max_iter:', max_iter)
        examined_lambda = ADlasso2(lmbd = lambd, device = device, echo = training_echo, alpha=alpha, max_iter=max_iter, loss_tol=loss_tol, weight_tol=weight_tol, gamma=gamma)  # set alpha, gamma, max_iter, tol?
        examined_lambda.fit(train_X, train_y, train_pvl)
        selected_set = examined_lambda.feature_set
        loss_history = examined_lambda.loss_
        error_history = examined_lambda.convergence_
        ...
        result_tup =  (percentage, prevalence_, train_prevalence, feature_number, 
                        auc, aupr, mcc, precision, f1, loss_history, error_history, 
                        loss_converge_history, loss_diff_history)
        return result_tup
...
def featureProperty(X, y, AD_object):
    ...
    for w in weights:
        if w > 0:
            selection.append('ADlasso_positive')
        elif w < 0:
            selection.append('ADlasso_negative')
        else:  # w == 0
