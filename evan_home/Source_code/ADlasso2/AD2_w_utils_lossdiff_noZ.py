...
def lambda_tuning_para_cpu(X_norm, Y, lmbdrange, device='cpu', training_echo=True,
    ...
    prevalence = get_prevalence(X_norm, celltype_indices)  # prevalence for class1 cells (屬於該celltype的細胞的prevalence)
    ...
    other_prevalence = get_prevalence(X_norm, other_indices)
    n_lambda = len(lmbdrange)
...
def lambda_tuning_cuda(X_norm, Y, lmbdrange, X_raw_count=None, device='cuda', training_echo=True,
...
def lambda_decision(result_dict, k=2, savepath=None, fig_width=8, fig_height=4):
    ...
    segment_variables = (xs, ys, dys_dt, ys_sl)
    ...
    return selected_lambda, candidate_lambdas, fig, segment_variables
