...
class ADlasso2():
    def __init__(self, lmbd=1e-5, max_iter=1000, tol=1e-4, alpha=0.01, loss_threshold=1e-2, gamma=0.1, device='cpu', echo=True):
        # original alpha = 0.1
        super().__init__()
        self.lmbd = lmbd
...
#         pvl = np.append(pvl,1)     # append 1 for representing bias prevalence
    ...
    def fit(self, X_input, Y, prevalence):
        ...
        for k in range(self.max_iter):
            ...
            status = 'OWL-QN did not convergence'
            print(status)
...
def lambda_tuning_para_ttsplit(X_norm, Y, lmbdrange, device='cpu', loss_threshold=0.05, training_echo=False,
                                max_iter=1000, tol=1e-2, alpha=0.01, gamma=0.1, n_jobs=1):
    class_content = np.unique(Y)
    if len(class_content) != 2:
        raise ValueError("This procedure allows only 2 classes.")
    ...
    def test_lambda(lambd):
        ...
        # Create new ADlasso object
        print('Lambda:', lambd)
        examined_lambda = ADlasso2(lmbd = lambd, loss_threshold=loss_threshold, device = device, echo = training_echo, alpha=alpha, max_iter=max_iter, gamma=gamma)  # set alpha, gamma, max_iter, tol?
        print('loss_threshold:', examined_lambda.loss_threshold)
        examined_lambda.fit(train_X, train_y, train_pvl)
        selected_set = examined_lambda.feature_set
        ...
        # loss converge history (as dict, key is iteration number)
    ...
    return metrics_dict, loss_converge_history_dict


def lambda_tuning_cuda(X_norm, Y, lmbdrange, device='cuda', training_echo=False,
                                max_iter=1000, tol=1e-4, alpha=0.01, gamma=0.1):
    class_content = np.unique(Y)
    if len(class_content) != 2:
        raise ValueError("This procedure allows only 2 classes.")
    ...
    for lmbd in lmbdrange:
        res = test_lambda(lmbd)
        tmpDF = pd.DataFrame(res, index=[lmbd])
        DF_results = pd.concat([DF_results, tmpDF], axis=0)
...
def lambda_tuning_MP(X_norm, Y, lmbdrange, device='cpu', training_echo=False, 
    ...
    #     print(res.get())
    #     print(type(res.get()))
    #     tmpDF = pd.DataFrame(res.get(), index=[0])  # res is a dict
    #     DF_result = pd.concat([DF_result, tmpDF], ignore_index=True)  # combine 1-D array tmpDF to DF_result

    # Create a multiprocessing Pool
    pool = mp.Pool(n_jobs)

    # Apply test_lambda to each value in lmbdrange
    results = [pool.apply_async(test_lambda, args=(lmbd,)) for lmbd in lmbdrange]
    ...
    DF_result = pd.DataFrame([result.get() for result in results])
