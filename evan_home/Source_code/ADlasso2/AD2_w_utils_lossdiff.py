...
class ADlasso2():
    def __init__(self, lmbd=1e-5, max_iter=1000, loss_tol=1e-6, weight_tol=1e-4, alpha=0.01, gamma=0.1, device='cpu', echo=True):
        # original alpha = 0.1
        super().__init__()
        self.lmbd = lmbd
...
#         pvl = np.append(pvl,1)     # append 1 for representing bias prevalence
    ...
    def fit(self, X_input, Y, prevalence):
        ...
        self.loss_history = {}
        self.loss_diff_history = {}
        # curr_loss = torch.tensor(float('inf'))
        curr_loss = torch.tensor(999)  # initialize curr_loss as a large number
        for k in range(self.max_iter):
        # for k in tqdm.tqdm(range(self.max_iter), desc="Iterations"):
            # print('iter:', k)
            prev_weight = weight
            prev_loss = curr_loss
            ...
            # document loss and loss difference by iteration
            self.loss_history[k] = curr_loss.item()
            self.loss_diff_history[k] = diff_loss

            # Convergence with loss difference
            if abs(diff_loss) <= self.loss_tol:
                self.stop_iter = k
                status = 'Convergence with loss difference, Device: ' + self.UsingDevice
                print(f'At iteration {k},', status)
                break

            # Convergence with loss threshold
            # if mini_loss < self.loss_threshold:
            #     status = 'Convergence with loss threshold'
            #     print(status)
            #     break
    
            # Convergence with weight difference
            # if diff_w <= self.weight_tol:
            #     # status = 'OWL-QN convergence'
            #     status = 'Convergence with weight difference, Device: ' + self.UsingDevice
            #     print(f'At iteration {k},', status)
            #     break
            ...
            status = 'OWL-QN did not convergence, Device: ' + self.UsingDevice
            print(status)
        ...
        if self.echo:
            print(status)
            # print('Converge history:`')
            # print(self.loss_history)
            # print('minimum epoch = ', self.n_iter_, '; minimum lost = ', self.loss_, '; diff weight = ', self.convergence_)
            print('minimum epoch = ', self.n_iter_, '; minimum lost = ', self.loss_, '; diff loss = ', diff_loss, '; diff weight = ', self.convergence_)
...
def lambda_tuning_para_cpu(X_norm, X_raw_count, Y, lmbdrange, device='cpu', training_echo=True,
                                max_iter=1000, loss_tol=1e-6, alpha=0.01, gamma=0.1, n_jobs=1):
    # formerly lambda_tuning_para_ttsplit()
    # Take in X_raw_count for Z-transformed X_norm
    print('Alpha:', alpha)
    print('Loss tolerance:', loss_tol)
    class_content = np.unique(Y)
    if len(class_content) != 2:
        raise ValueError("This procedure allows only 2 classes.")
    ...
    y = np.array([0 if yi == class_content[0] else 1 for yi in Y])
    # Target prevalence: prevalence with one cell type
    celltype_indices = [idx for idx, label in enumerate(y) if label == 1]  # from enumerate(Y) change to enumerate(y)
    prevalence = get_prevalence(X_raw_count, celltype_indices)  # prevalence for class1 cells (屬於該celltype的細胞的prevalence)
    # Other prevalence: prevalence in other cells apart from target celltype
    other_indices = [idx for idx, label in enumerate(y) if label == 0]
    other_prevalence = get_prevalence(X_raw_count, other_indices)
    n_lambda = len(lmbdrange)
    
    # Evaluation metrics
    metrics = ['Percentage', 'Prevalence', 'Other_prevalence', 'Feature_number', 'AUC', 'AUPR', 'MCC', 
    ...
    def test_lambda(lambd):
        start = time.time()
        # Create new ADlasso object
        print('Lambda:', round(lambd, 6), 'starting at', datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Max_iter:', max_iter)
        examined_lambda = ADlasso2(lmbd = lambd, device = device, echo = training_echo, alpha=alpha, max_iter=max_iter, loss_tol=loss_tol, gamma=gamma)
        examined_lambda.fit(X_norm, y, prevalence)
        selected_set = examined_lambda.feature_set
        ...
        if np.sum(selected_set) >= 1:
            ...
            other_pvl_ = np.median(other_prevalence[selected_set != 0])
            # train_prevalence = np.median(train_pvl[selected_set != 0])
            # new LR model and evaluation
            norm_LR = LogisticRegression(penalty=None)  # penalty='none'
            perf = evaluation(X_norm, y, X_norm, y, examined_lambda, norm_LR)
            auc = perf['AUC']
            aupr = perf['AUPR']
            mcc = perf['MCC']
        ...
        # loss converge history (as dictionary, key is iteration number)
        loss_converge_history = examined_lambda.loss_history
        loss_diff_history = examined_lambda.loss_diff_history
        end = time.time()
        print('lambda is : {lmb}, cost : {tm} min'.format(lmb = lambd, tm = round((end - start)/60, 3)))
        print('==========')
        
        return (percentage, prevalence_, other_pvl_, feature_number, 
                auc, aupr, mcc, precision, f1, loss_history, error_history, 
                loss_converge_history, loss_diff_history)
    ...
    loss_converge_history_dict = dict()
    loss_diff_history_dict = dict()
    for i, result in enumerate(results):
        metrics_dict['Percentage'][i] = result[0]
        metrics_dict['Prevalence'][i] = result[1]
        metrics_dict['Other_prevalence'][i] = result[2]
        ...
        metrics_dict['Precision'][i] = result[7]
        metrics_dict['F1 score'][i] = result[8]
        metrics_dict['loss_history'][i] = result[9]
        metrics_dict['error_history'][i] = result[10]
        loss_converge_history_dict[i] = result[11]
        loss_diff_history_dict[i] = result[12]

    metrics_dict['log_lambda_range'] = np.log(lmbdrange)

    return metrics_dict, loss_converge_history_dict, loss_diff_history_dict


def lambda_tuning_cuda(X_norm, X_raw_count, Y, lmbdrange, device='cuda', training_echo=True,
                                max_iter=1000, loss_tol=1e-6, alpha=0.01, gamma=0.1):
    # Take in X_raw_count for Z-transformed X_norm
    print('Alpha:', alpha)
    print('Loss tolerance:', loss_tol)
    class_content = np.unique(Y)
    ...
    # Target prevalence: prevalence with one cell type
    celltype_indices = [idx for idx, label in enumerate(y) if label == 1]  # from enumerate(Y) change to enumerate(y)
    prevalence = get_prevalence(X_raw_count, celltype_indices)
    # Other prevalence: prevalence in other cells apart from target celltype
    other_indices = [idx for idx, label in enumerate(y) if label == 0]
    other_prevalence = get_prevalence(X_raw_count, other_indices)
    n_lambda = len(lmbdrange)
    ...
    def test_lambda(lambd):
        start = time.time()
        # Create new ADlasso object
        print('Testing lambda:', round(lambd, 6), 'starting at', datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'Max_iter:', max_iter)
        examined_lambda = ADlasso2(lmbd = lambd, device = device, echo = training_echo, alpha=alpha, max_iter=max_iter, loss_tol=loss_tol, gamma=gamma)
        examined_lambda.fit(X_norm, y, prevalence)
        selected_set = examined_lambda.feature_set
        loss_history = examined_lambda.loss_
        ...
        # log lambda range
        # metrics_dict['log_lambda_range'] = np.log(lmbdrange)
        # metrics_dict['log_lambda_range'] = np.log(lambd)
        ...
        # return metrics_dict, loss_converge_history, loss_diff_history
        result_tup =  (percentage, prevalence_, other_pvl_, feature_number, 
                        auc, aupr, mcc, precision, f1, loss_history, error_history, 
                        loss_converge_history, loss_diff_history)
        return result_tup
    
    ### test each lambda in for loop
    DF_results = pd.DataFrame(columns=metrics)  # save 11 metrics: percentage to error_history
    loss_converge_history_dict = dict()
    loss_diff_history_dict = dict()
    for i, lmbd in enumerate(lmbdrange):
        result_tup = test_lambda(lmbd)
        try:
            tmp_dict = dict(zip(metrics, result_tup[:-2]))  # get 11 metrics
            DF_results.loc[lmbd] = tmp_dict
        except:
            print(f'tmpDict error at {lmbd}')
        
        metrics_dict['Percentage'][i] = result_tup[0]
        metrics_dict['Prevalence'][i] = result_tup[1]
        metrics_dict['Other_prevalence'][i] = result_tup[2]
        # metrics_dict['Train_prevalence'][i] = result_tup[2]
        metrics_dict['Feature_number'][i] = result_tup[3]
        metrics_dict['AUC'][i] = result_tup[4]
        metrics_dict['AUPR'][i] = result_tup[5]
        metrics_dict['MCC'][i] = result_tup[6]
        metrics_dict['Precision'][i] = result_tup[7]
        metrics_dict['F1 score'][i] = result_tup[8]
        metrics_dict['loss_history'][i] = result_tup[9]
        metrics_dict['error_history'][i] = result_tup[10]
        loss_converge_history_dict[i] = result_tup[11]
        loss_diff_history_dict[i] = result_tup[12]

    metrics_dict['log_lambda_range'] = np.log(lmbdrange)

    return DF_results, metrics_dict, loss_converge_history_dict, loss_diff_history_dict
...
def lambda_tuning_viz(result_dict, metric, savepath=None, fig_width=8, fig_height=4):
    ...
    elif isinstance(result_dict, pd.DataFrame):
        lmbd_range = np.exp(result_dict['log_lambda_range'].to_numpy())
        metrics_recode = result_dict[metric].to_numpy()
        pvl_recode = result_dict['Prevalence'].to_numpy()
    else:
        raise ValueError("result_dict must be a dict or a pandas DataFrame.")
    ...
    if metric in ['Feature_number', 'error_history']:
        ax1.set(yscale="log")
    if savepath:
        plt.savefig(savepath, dpi=300)
    return fig

def lambda_decision(result_dict, k=2, savepath=None, fig_width=8, fig_height=4):
    if isinstance(result_dict, dict):
        ...
        lmbd_range = np.exp(result_dict['log_lambda_range'].to_numpy())
        loss_recode = result_dict['loss_history'].to_numpy()
        pvl_recode = result_dict['Prevalence'].to_numpy()
    ...
    ax2.errorbar(lmbd_range, pvl_recode, marker='o', c='#FFAA33', linestyle='--', label='Prevalence', zorder=1); ax2.legend(loc='lower right')
    ...
    return selected_lambda, candidate_lambdas, fig
...
def featureProperty(X, y, AD_object):
    ...
    weights = AD_object.w
    # tendency = [0 if w < 0 else 1 for w in weights]  # what to do when w is 0 (majority of unselected features)
    selection = []
    for w in weights:
        if w > 0:
            selection.append('PreLect_positive')
        elif w < 0:
            selection.append('PreLect_negative')
        else:  # w == 0
            selection.append('No selected')
    C0head = 'prevalence_' + str(list(AD_object.classes_.keys())[0])  # class 0 prevalence
    C1head = 'prevalence_' + str(list(AD_object.classes_.keys())[1])  # class 1 prevalence
