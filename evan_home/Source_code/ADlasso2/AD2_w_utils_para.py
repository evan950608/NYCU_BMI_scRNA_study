...
def lambda_tuning(X_input, X_raw, Y, lmbdrange, k_fold, outdir, device='cpu', training_echo=False,
                  max_iter=100, tol=1e-4, alpha=0.001, gamma=0.1):
    class_content = np.unique(Y)
    if len(class_content) != 2:
    ...
    y = np.array([0 if yi == class_content[0] else 1 for yi in Y])
    # TODO: modify sample_index for prevalence calculation
    ...
    for i in range(n_lambda):
        ...
        for train_ix, test_ix in kfold.split(X_input, y):
            ...
            # calculate new prevalence and create new ADlasso object
...
def lambda_tuning_evan(X_norm, Y, lmbdrange, device='cpu', training_echo=False,
    ...
    metrics = ['Percentage', 'Prevalence', 'Train_prevalence', 'Train_prevalence_global', 
               'Feature_number', 'AUC', 'AUPR', 'MCC', 'loss_history', 'error_history']
    metrics_dict = dict()
    for m in metrics :
        # metrics_dict[m] = np.zeros((n_lambda, k_fold))
...
def lambda_tuning_parallel(X_norm, Y, lmbdrange, device='cpu', training_echo=False,
    ...
    # Remove pairwiseMCC
    metrics = ['Percentage', 'Prevalence', 'Train_prevalence', 'Train_prevalence_global', 
               'Feature_number', 'AUC', 'AUPR', 'MCC', 'Precision', 'Precision met', 'F1 score', 'loss_history', 'error_history']
    metrics_dict = dict()
    for m in metrics :
        # metrics_dict[m] = np.zeros((n_lambda, k_fold))
        metrics_dict[m] = np.zeros(n_lambda)
    ...
    def test_lambda(lambd):
        ...
        if np.sum(selected_set) > 1:
            ...
            precision_met = perf['Precision met']
            f1 = perf['F1 score']
        else:
            feature_number = 0
            percentage = 0
            ...
            precision = precision_met = f1 = 0

        end = time.time()
        print('lambda is : {lmb}, cost : {tm} min'.format(lmb = lambd, tm = round((end - start)/60, 3)))
        print('==========')
        
        return (percentage, prevalence_, train_prevalence, train_prevalence_global, feature_number, auc, aupr, mcc, precision, precision_met, f1, loss_history, error_history)
    ...
    for i, result in enumerate(results):
        ...
        metrics_dict['Feature_number'][i] = result[4]
        metrics_dict['AUC'][i] = result[5]
        metrics_dict['AUPR'][i] = result[6]
        metrics_dict['MCC'][i] = result[7]
        metrics_dict['Precision'][i] = result[8]  # error here
        metrics_dict['Precision met'][i] = result[9]
        metrics_dict['F1 score'][i] = result[10]
        metrics_dict['loss_history'][i] = result[11]
        metrics_dict['error_history'][i] = result[12]
...
def evaluation(x_train, y_train, x_test, y_test, AD_object, classifier):
    ...
    return {"AUC" : auroc, "AUPR" : aupr, "MCC" : mcc, 'Precision': precision, 'Precision met': precision_met, 'F1 score': f1}
